...
float myKmeans(InputArray _data, int K,
	...
	int flags, OutputArray _centers)
{
	...
	Vec2f* box = &_box[0];
	float best_compactness = DBL_MAX, compactness = 0;
	RNG& rng = theRNG();
	int a, iter, i, j, k;
	...
	{
		float max_center_shift = DBL_MAX;
		for (iter = 0;;)
		{
				...
				for (k = 0; k < K; k++)
				{
					...
					for (i = 0; i < N; i++)
					{
						if (labels[i] != max_k)
							continue;
						sample = data.ptr<float>(i);
						float dist = normL2Sqr_(sample, _old_center, dims);
